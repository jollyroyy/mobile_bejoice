...
function CareersModal({ onClose }) {
  ...
  async function handleSubmit(ev) {
    ...
    const position = form.position === 'Other' ? clampHelper(form.otherPosition, 100) : form.position
    ...
    try {
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
        ...
        from_name: form.name,
        subject: `[Careers] ${position} - ${form.name}`,
        message: body,
      }, EMAILJS_PUBLIC_KEY)
      setSubmitted(true)
    } catch (err) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'rgba(3,3,8,0.85)', backdropFilter: 'blur(12px)',
        display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px'
      }}
    >
      <div style={{
        width: '100%', maxWidth: 520, background: '#0f172a', border: '1.5px solid rgba(91,194,231,0.3)',
        borderRadius: 24, padding: '40px', position: 'relative', overflow: 'hidden', color: '#fff'
      }}>
        {submitted ? (
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ fontFamily: "var(--font-bebas)", fontSize: '2.5rem', color: '#5BC2E7' }}>Application Received!</h2>
            <p style={{ fontFamily: "var(--font-plus-jakarta)", opacity: 0.8, marginTop: '10px' }}>We'll be in touch soon.</p>
            <button onClick={onClose} style={{ marginTop: '20px', padding: '12px 24px', background: '#5BC2E7', borderRadius: '8px', color: '#000', fontWeight: 700 }}>Close</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h2 style={{ fontFamily: "var(--font-bebas)", fontSize: '2.5rem', marginBottom: '10px' }}>Join <span style={{ color: '#5BC2E7' }}>Our Team</span></h2>
            <input type="text" placeholder="Full Name" className="cm-inp" onChange={e => set('name', e.target.value)} required />
            <input type="email" placeholder="Email Address" className="cm-inp" onChange={e => set('email', e.target.value)} required />
            <input type="text" placeholder="Phone Number" className="cm-inp" onChange={e => set('phone', e.target.value)} required />
            <select className="cm-inp" onChange={e => set('position', e.target.value)} required style={{ appearance: 'none', background: 'rgba(255,255,255,0.05)' }}>
              <option value="">Select Position</option>
              {POSITIONS.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
            <textarea placeholder="Tell us about yourself..." className="cm-inp" rows="3" onChange={e => set('message', e.target.value)}></textarea>
            <button type="submit" disabled={submitting} className="cm-sub" style={{ background: '#5BC2E7', color: '#000', padding: '16px', borderRadius: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              {submitting ? 'Sending...' : 'Submit Application'}
            </button>
          </form>
        )}
        <button onClick={onClose} style={{ position: 'absolute', top: 15, right: 15, color: 'rgba(255,255,255,0.5)' }}>✕</button>
      </div>
      <style>{`
        .cm-inp { width:100%; padding:14px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1); border-radius:10px; color:#fff; font-family:var(--font-plus-jakarta); outline:none; }
        .cm-inp:focus { border-color:#5BC2E7; background:rgba(255,255,255,0.06); }
      `}</style>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div style={{
        ...
        border: '1.5px solid rgba(91, 194, 231, 0.25)',
        ...
        <div style={{
          ...
          {policy.updated && (
            <p style={{
              fontFamily: "var(--font-plus-jakarta)", fontSize: '12px',
              color: '#5BC2E7', opacity: 0.8, marginTop: '8px',
          ...
          {sections.map((s, i) => (
            <div key={i} style={{ marginBottom: '40px' }}>
              <h3 style={{
                ...
                color: '#5BC2E7', marginBottom: '16px', letterSpacing: '0.03em'
              }}>
                {s.title}
              </h3>
              <div style={{
                fontFamily: "var(--font-plus-jakarta)", fontSize: '15px',
        ...
        <div style={{
          ...
          <button
            onClick={onClose}
            style={{
              padding: '12px 32px', borderRadius: '12px', border: '1px solid rgba(91,194,231,0.4)',
              background: 'rgba(91,194,231,0.1)', color: '#5BC2E7',
              fontFamily: "var(--font-plus-jakarta)", fontWeight: 700, cursor: 'pointer',
              transition: 'all 0.3s'
            }}
            onMouseEnter={e => { e.target.style.background = '#5BC2E7'; e.target.style.color = '#000' }}
            onMouseLeave={e => { e.target.style.background = 'rgba(91,194,231,0.1)'; e.target.style.color = '#5BC2E7' }}
          >
            {isAr ? 'إغلاق' : 'Close'}
          </button>
        </div>
        ...
        <button
          onClick={onClose}
          style={{
            position: 'absolute', top: 30, right: 30, width: 44, height: 44,
...
export default function Footer({ onWhyClick }) {
  ...
  return (
    <footer className="relative border-t pt-10 md:pt-20 pb-10 px-6 md:px-12 lg:px-24 overflow-hidden cv-section cv-footer" style={{ borderColor: 'rgba(255,255,255,0.08)', background: '#0a192f' }}>
      {/* Premium Background Accents */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(circle at 50% 100%, rgba(91,194,231,0.08) 0%, transparent 70%)' }}/>
      
      <div className="max-w-7xl mx-auto relative">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 md:gap-12 lg:gap-16 mb-12 md:mb-16 lg:mb-20">
          
          {/* Logo Section */}
          <div className="col-span-2 md:col-span-3 lg:col-span-2 flex flex-col items-center lg:items-start">
            <img
              src="/bejoice-logo-group.png"
              alt="Bejoice"
              loading="lazy" decoding="async"
              style={{ height: 'auto', width: '280px', maxWidth: '100%', objectFit: 'contain', opacity: 0.95 }}
            />
          </div>

          {/* Links Sections */}
          {Object.entries(footerLinks).map(([category, items]) => (
            <div key={category} className="footer-col" style={{ textAlign: isAr ? 'right' : 'left' }}>
              <div style={{ 
                fontFamily: isAr ? "var(--font-cairo)" : "var(--font-bebas)", 
                ...
                <div style={{ position: 'absolute', bottom: '-8px', left: isAr ? 'auto' : 0, right: isAr ? 0 : 'auto', width: '24px', height: '2px', background: 'var(--gold)', borderRadius: '1px' }}></div>
              </div>
                  ...
                  <li key={item}>
                    {item === 'Careers' ? (
                      <button
                        ...
                        style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: '15px', color: 'rgba(255,255,255,0.6)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, transition: 'all 0.3s' }}
                      ...
                      <a
                        href={item === 'Track Shipment' ? 'https://www.track-trace.com/' : (item === 'Get a Quote' || item === 'Contact Us') ? '#contact' : '#'}
                        onClick={(e) => {
                          if (item !== 'Track Shipment') {
                            e.preventDefault()
                            const target = item === 'Get a Quote' || item === 'Contact Us' ? 'contact' : ''
                            const el = document.getElementById(target)
                            if (el) window.__lenis?.scrollTo(el) || el.scrollIntoView({ behavior: 'smooth' })
                          }
                        }}
                        style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: '15px', color: 'rgba(255,255,255,0.6)', textDecoration: 'none', transition: 'all 0.3s' }}
                        onMouseEnter={e => e.currentTarget.style.color = '#fff'}
                        onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.6)'}
                      >
          ...
          {/* Contact Column */}
          <div className="footer-contact-col" style={{ textAlign: isAr ? 'right' : 'left' }}>
            <div style={{ 
              fontFamily: isAr ? "var(--font-cairo)" : "var(--font-bebas)", 
              fontSize: isAr ? '1.1rem' : '1.3rem',
              letterSpacing: isAr ? 0 : '0.15em', 
              ...
              {isAr ? ar.footer.categories['Contact Us'] : 'Contact Us'}
              <div style={{ position: 'absolute', bottom: '-8px', left: isAr ? 'auto' : 0, right: isAr ? 0 : 'auto', width: '24px', height: '2px', background: 'var(--gold)', borderRadius: '1px' }}></div>
            </div>
            <ul className="space-y-6">
              <li style={{ display: 'flex', alignItems: 'center', gap: '12px', justifyContent: 'inherit' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(91,194,231,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#5BC2E7" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.65 3.4 2 2 0 0 1 3.62 1.22h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                </div>
                <a href="tel:+966138233461" style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: '15px', color: '#fff', textDecoration: 'none' }}>+966 13 823 3461</a>
              </li>
              <li style={{ display: 'flex', alignItems: 'center', gap: '12px', justifyContent: 'inherit' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(91,194,231,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#5BC2E7" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                </div>
                <a href="mailto:info@bejoiceshipping-ksa.com" style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: '15px', color: '#fff', textDecoration: 'none', wordBreak: 'break-all' }}>info@bejoiceshipping-ksa.com</a>
              </li>
              <li style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', justifyContent: 'inherit' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(91,194,231,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '2px' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#5BC2E7" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                </div>
                <span style={{ fontFamily: isAr ? "var(--font-cairo)" : "var(--font-plus-jakarta)", fontSize: '14px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.6 }}>
                  {isAr ? ar.footer.address : (
                    <>Al Raja Avenue, Block A, 1st floor<br />Office No. 2, Dammam 32234, KSA</>
                  )}
                </span>
              </li>
            </ul>
        ...
        {/* Divider */}
        <div style={{ height: '1px', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.1), transparent)', margin: '40px 0' }} />
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
          ...
          >
            ...
            <div>
              <div style={{ fontFamily: "var(--font-bebas)", fontSize: 'clamp(15px,1.4vw,18px)', letterSpacing: '0.18em', color: '#ffffff', lineHeight: 1.1 }}>BEJOICE SHIPPING LLC</div>
              <div style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: 'clamp(11px,1vw,13px)', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(0,169,255,0.75)', marginTop: 3, fontWeight: 600 }}>{isAr ? ar.footer.followLinkedIn : 'Follow on LinkedIn'}</div>
            </div>
          ...
          <a
            ...
            onMouseEnter={e => {
              ...
              e.currentTarget.style.boxShadow = '0 0 32px rgba(225,48,108,0.18), inset 0 0 20px rgba(225,48,108,0.05)'
            }}
            onMouseLeave={e => {
          ...
          >
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', overflow: 'hidden' }}>
              <div style={{ height: '100%', background: 'linear-gradient(90deg, transparent, rgba(225,48,108,0.8), transparent)', animation: 'igSweep 3.5s ease-in-out infinite' }} />
            </div>
            <div style={{ width: 36, height: 36, borderRadius: 6, flexShrink: 0, background: 'linear-gradient(135deg, #833AB4 0%, #C13584 50%, #E1306C 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 12px rgba(225,48,108,0.4)' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="#ffffff">
            ...
            <div>
              <div style={{ fontFamily: "var(--font-bebas)", fontSize: 'clamp(15px,1.4vw,18px)', letterSpacing: '0.18em', color: '#ffffff', lineHeight: 1.1 }}>BEJOICE INSTAGRAM</div>
              <div style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: 'clamp(11px,1vw,13px)', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(255,120,160,0.75)', marginTop: 3, fontWeight: 600 }}>{isAr ? ar.footer.followInstagram : 'Follow on Instagram'}</div>
            </div>
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" style={{ marginLeft: 4, flexShrink: 0, opacity: 0.6 }}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-10 border-t border-white/5">
          <p style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: '14px', color: 'rgba(255,255,255,0.4)' }}>
            {isAr ? ar.footer.rights : '© Bejoice Shipping Company. All Rights Reserved.'}
          </p>
          <div className="flex flex-wrap justify-center gap-x-8 gap-y-4">
            {Object.keys(POLICIES).map(title => (
              <button
                key={title}
                onClick={() => setOpenPolicy(title)}
                style={{ 
                  fontFamily: isAr ? "var(--font-cairo)" : "var(--font-plus-jakarta)", 
